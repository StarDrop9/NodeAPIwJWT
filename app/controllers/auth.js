 ...
 * @param {Object} item - user object that contains created id
 * @param {Object} userInfo - user object
 * may need to go back to userInfo for verification probably so...
 */
const returnPostToken = (item, postInfo) => {
  if (process.env.NODE_ENV !== 'production') {
    postInfo.verification = item.verification
  }
  const data = {
    token: generateToken(item._id),
    post: postInfo
...
/* exports.posts = async (req, res) => {
  try {
    ...
    if (true==true) {
      // const item = await registerUser(req)
      const postInfo = setPostInfo(item)
      const response = returnPostToken(item, postInfo)
      // emailer.sendRegistrationEmailMessage(locale, item)
      res.status(201).json(response)
    }
  } catch (error) {
